...
export async function getScanSummary(scanId: string): Promise<ScanSummary> {
  const res = await fetch(`${API_BASE}/api/results/${scanId}/summary`);
  return res.json();
}

export async function getEndpoints(scanId: string): Promise<EndpointResult[]> {
  const res = await fetch(`${API_BASE}/api/results/${scanId}/endpoints`);
  return res.json();
}
...
export async function getDiagrams(scanId: string): Promise<DiagramFile[]> {
  const res = await fetch(`${API_BASE}/api/diagrams/${scanId}`);
  return res.json();
}
...
export async function uploadCloudwatchLogs(
  ...
  const res = await fetch(`${API_BASE}/api/analytics/import?${params}`, {
    method: "POST",
...
export async function getApiAnalytics(
  ...
  const res = await fetch(`${API_BASE}/api/analytics/timeline?${params}`);
  return res.json();
}
...
export async function getAllProjectAnalytics(
  ...
  const params = new URLSearchParams({ projectName });
  const res = await fetch(`${API_BASE}/api/analytics/all?${params}`);
  if (!res.ok) return [];
...
export async function getProjectBatches(
  ...
  const params = new URLSearchParams({ projectName });
  const res = await fetch(`${API_BASE}/api/analytics/batches?${params}`);
  if (!res.ok) return [];
  const data = await res.json();
  return Array.isArray(data) ? data : [];
}
...
export async function getBatchLogs(
  ...
  const res = await fetch(`${API_BASE}/api/analytics/batch?${params}`);
  if (!res.ok) return [];
  const data = await res.json();
  return Array.isArray(data) ? data : [];
}
...
export async function getAiStatus(scanId: string): Promise<AiStatus> {
  const res = await fetch(`${API_BASE}/api/ai/status/${scanId}`);
...
export async function importAiIssues(
  ...
  const res = await fetch(`${API_BASE}/api/ai/import/${scanId}`, {
    method: "POST",
  });
  if (!res.ok) {
...
export async function deleteAiIssues(scanId: string): Promise<void> {
  const res = await fetch(`${API_BASE}/api/ai/issues/${scanId}`, {
    method: "DELETE",
  });
...
export async function prepareFix(
  ...
  const res = await fetch(`${API_BASE}/api/ai/prepare-fix`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      scanId,
...
export async function getFixQueue(scanId: string): Promise<FixQueueEntry[]> {
  const res = await fetch(`${API_BASE}/api/ai/fix-queue/${scanId}`);
  if (!res.ok) return [];
  return res.json();
}
...
export async function bulkPrepareFix(
  ...
  const res = await fetch(`${API_BASE}/api/ai/bulk-prepare-fix`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      scanId,
...
export async function getTestProjects(): Promise<string[]> {
  const res = await fetch(`${API_BASE}/api/test/projects`);
  return res.json();
}
...
export async function getTestEndpoints(
  ...
  const params = new URLSearchParams({ projectName });
  const res = await fetch(`${API_BASE}/api/test/endpoints?${params}`);
  return res.json();
}
...
export async function getApiTestHistory(
  ...
  const params = new URLSearchParams({ projectName, endpointPath, httpMethod });
  const res = await fetch(`${API_BASE}/api/test/history?${params}`);
  return res.json();
}
...
export async function startLiveTest(
  ...
  const res = await fetch(`${API_BASE}/api/test/run-live`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(request),
  });
...
export async function cancelLiveTest(runId: string): Promise<void> {
  await fetch(`${API_BASE}/api/test/run-live/${runId}`, { method: "DELETE" });
}

export function getLiveTestWsUrl(liveRunId: string): string {
...
export async function saveMonitorConfig(
  ...
  const res = await fetch(`${API_BASE}/api/monitor/config`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
